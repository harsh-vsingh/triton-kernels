...
from utils import pointwise_launch_config, validate_bias, validate_binary
...
def add(x: torch.Tensor, y: torch.Tensor, out: torch.Tensor = None) -> torch.Tensor:
    """
    Elementwise add.
...
def add_and_relu(x: torch.Tensor, y: torch.Tensor, out: torch.Tensor = None) -> torch.Tensor:
    """
...
def _bias_and_gelu_kernel(
    ...
    out = (
        0.5
        * tmp
        * (1.0 + tl.extra.libdevice.tanh(0.7978845608028654 * (tmp + 0.044715 * tmp * tmp * tmp)))
    )
    tl.store(out0_ptr + offsets, out, mask=mask)


def bias_and_gelu(x: torch.Tensor, bias: torch.Tensor, out: torch.Tensor = None) -> torch.Tensor:
    """
    ...
    out = torch.empty_like(x) if out is None else out
    _bias_and_gelu_kernel[grid](
        x, bias, out, n, hidden_dim, BLOCK_SIZE=block_size, num_warps=num_warps
    )
    return out
...
def swiglu(x: torch.Tensor, mul: torch.Tensor, out: torch.Tensor = None) -> torch.Tensor:
    """
    ...
    out = torch.empty_like(x) if out is None else out
    _swiglu_kernel[grid](x, mul, out, n, BLOCK_SIZE=block_size, num_warps=num_warps)
    return out
...
def bias_silu_mult(
    x: torch.Tensor, bias: torch.Tensor, mult: torch.Tensor, out: torch.Tensor = None
) -> torch.Tensor:
    """
    ...
    out = torch.empty_like(x) if out is None else out
    _bias_silu_mult_kernel[grid](
        x, bias, mult, out, n, hidden_dim, BLOCK_SIZE=block_size, num_warps=num_warps
    )
    return out
