...
from  utils import validate_layernorm, validate_rms_norm, validate_binary
...
def _layernorm_kernel_medium(
    ...
    for col_idx in tl.range(0, n_cols, BLOCK_SIZE, num_stages=num_stages):
        ...
        x = tl.load(x_ptr + offsets, mask=mask, other=0.0,).to(tl.float32)
        gamma = tl.load(gamma_ptr + col_idx + offset_range, mask=mask,).to(tl.float32)
        beta = tl.load(beta_ptr + col_idx + offset_range, mask=mask,).to(tl.float32)
...
def _rms_kernel_merge(
    ...
    partial_sumsq = tl.load(partial_sumsq_ptr + row_start + offsets, mask=mask, other=0.0).to(tl.float32)
...
def _rms_kernel_final(
    ...
    for chunk in tl.range(chunk_id, chunks_per_row, programs_per_row, num_stages=num_stages):
        ...
        x = tl.load(x_ptr + offsets, mask=mask, other=0.0).to(tl.float32)
        gamma = tl.load(gamma_ptr + chunk * BLOCK_SIZE + offset_range, mask=mask, other=0.0).to(tl.float32)
...
def _residual_layernorm_kernel_partial(
    ...
    for chunk in tl.range(chunk_id, chunks_per_row, programs_per_row, num_stages=num_stages):
        ...
        residual = tl.load(residual_ptr + block_start + offsets, mask=mask, other=0.0,)
        z = (x + residual).to(tl.float32)
        chunk_mean = z
        chunk_m2 = tl.zeros([BLOCK_SIZE], tl.float32)
        chunk_count = tl.where(mask, 1.0, 0.0)
